...
React + Vite + Tailwind CSS 프론트엔드, Firebase(Auth / Firestore / Realtime Database) 백엔드로 구성되어 있고,
Cloudflare Pages에 정적 배포합니다. **Firebase Storage는 사용하지 않아 Spark(무료) 플랜만으로 100% 무료로 운영할 수 있습니다.**
...
├ firebase/     Firebase 초기화 및 Auth·Firestore·Presence 래퍼 함수
...
3. **Realtime Database** → 만들기 (테스트/잠금 모드 아무거나 선택, 규칙은 아래에서 배포로 덮어씀)
4. Firebase CLI로 보안 규칙 배포:
   ```bash
...
- **이미지 전송 없음**: Firebase Storage를 쓰지 않기로 하면서 이미지 첨부 기능은 제거했습니다. 이모지로 대신하거나, 나중에 필요해지면 Storage를 다시 붙이거나 외부 이미지 링크(URL)를 붙여넣는 방식으로 확장할 수 있습니다.
...
- 이모지 선택기, 메시지 삭제(soft delete), 답장, `@닉네임` 멘션(자동완성 포함), 메시지 검색
- XSS 방어(DOMPurify), 입력값 검증, 클라이언트 사이드 스팸 방지, Firestore/RTDB 보안 규칙
- Firebase Storage 미사용 — Authentication + Firestore + Realtime Database만으로 Spark(무료) 플랜에서 완전히 무료로 동작
