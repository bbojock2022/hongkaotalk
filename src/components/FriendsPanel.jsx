...
export default function FriendsPanel({ user, currentRoom, onOpenDM }) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {message && <p className="text-xs text-gray-400 px-2 mb-2">{message}</p>}

      {searchResults.length > 0 && (
        <div className="mb-2 space-y-1 px-1">
          <p className="text-xs text-gray-500 px-2">검색 결과</p>
          {searchResults.map((r) => (
      ...
      <div className="flex-1 overflow-y-auto px-1">
        <p className="text-xs text-gray-500 px-2 py-1">내 친구 — {friends.length}</p>
